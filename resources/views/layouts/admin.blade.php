...
<head>
  ...
  {{--
    ┌──────────────────────────────────────────────────────────────┐
    │  BARU — layouts/admin.blade.php                              │
    │  Layout khusus untuk portal Level 2 (Divisi Umum).          │
    │  Mewarisi style yang sama dengan layout karyawan (app.blade) │
    │  namun memiliki sidebar menu berbeda sesuai role admin.      │
    └──────────────────────────────────────────────────────────────┘
  --}}
  <style>
    ...
    /* Stat card khusus (pending indicator) */
    ...
    /* Detail card */
    .detail-section {
      background:#F8FAFC; border-radius:12px; padding:20px 24px; margin-bottom:16px;
    }
    .detail-label {
  ...
  <aside class="sidebar">
    <div class="sidebar-header">
      ...
      <div class="sidebar-brand">
        ...
        <div class="brand-sub">Divisi Umum</div>
      </div>
    </div>
    ...
    <div class="sidebar-user">
      ...
      <div>
        ...
        <span class="role-badge-admin"><i class="bi bi-shield-check"></i> Admin Gudang</span>
      </div>
    </div>
    ...
    <nav class="sidebar-menu">
      ...
      <a href="{{ route('admin.pemakaian.index') }}"
        ...
        @if($badgePemakaian > 0)
          <span style="margin-left:auto;background:rgba(245,158,11,.9);color:#fff;font-size:10px;
                       font-weight:700;padding:2px 7px;border-radius:20px">{{ $badgePemakaian }}</span>
        @endif
      </a>
      ...
      <a href="{{ route('admin.pengadaan.index') }}"
        ...
        @if($badgePengadaan > 0)
          <span style="margin-left:auto;background:rgba(245,158,11,.9);color:#fff;font-size:10px;
                       font-weight:700;padding:2px 7px;border-radius:20px">{{ $badgePengadaan }}</span>
        @endif
      </a>
    </nav>
...
{{-- ── GLOBAL CONFIRMATION MODAL (sama persis dengan karyawan) ── --}}
  ...
  ">
    ...
    <div style="display:flex;gap:12px;justify-content:center;">
      <button onclick="hideConfirm()" style="
        ...
        font-weight:600;color:#64748b;cursor:pointer; transition:all .15s ease;
...
function doLogout(formId) {
  showConfirm({
    ...
    icon: 'bi-box-arrow-right', iconColor: '#DC2626',
    confirmText: 'Ya, Keluar', confirmClass: 'confirm-btn-danger',
...
function showConfirm(opts) {
  ...
  iconWrap.style.background = iconColor + '1a';
  var confirmClass = opts.confirmClass || 'confirm-btn-primary';
  if (confirmClass === 'confirm-btn-danger')        { btn.style.background = '#DC2626'; btnIcon.className = 'bi bi-x-lg'; }
  else if (confirmClass === 'confirm-btn-success')  { btn.style.background = '#059669'; btnIcon.className = 'bi bi-check-lg'; }
  else if (confirmClass === 'confirm-btn-warning')  { btn.style.background = '#D97706'; btnIcon.className = 'bi bi-check-lg'; }
  else { btn.style.background = '#0055A5'; btnIcon.className = 'bi bi-check-lg'; }
  if (opts.confirmIcon) btnIcon.className = 'bi ' + opts.confirmIcon;
  _confirmCallback = opts.onConfirm || null;
  btn.onclick = function() { hideConfirm(); if (_confirmCallback) setTimeout(_confirmCallback, 120); };
