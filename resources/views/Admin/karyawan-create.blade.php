...
@section('content')
{{--
  BARU — admin/karyawan-create.blade.php
  Form tambah akun karyawan baru oleh Divisi Umum.
  Role otomatis di-set ke 'karyawan' oleh controller.
--}}
        ...
        <div class="form-grid-2">
          <div class="form-group">
            ...
            <input class="form-control {{ $errors->has('name')?'is-invalid':'' }}"
                   type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Budi Santoso"/>
            @error('name')<div class="form-error">{{ $message }}</div>@enderror
          </div>
          <div class="form-group">
            <label>Email <span class="req">*</span></label>
            <input class="form-control {{ $errors->has('email')?'is-invalid':'' }}"
                   type="email" name="email" value="{{ old('email') }}" required placeholder="email@bps.go.id"/>
          ...
          <div class="form-group">
            ...
            <div class="input-pwd-wrap">
              <input class="form-control {{ $errors->has('password')?'is-invalid':'' }}"
                     type="password" name="password" id="pwd-new" placeholder="Minimal 8 karakter"/>
          ...
          <div class="form-group">
            ...
            <div class="input-pwd-wrap">
              <input class="form-control" type="password" name="password_confirmation" id="pwd-confirm" placeholder="Ulangi password"/>
          ...
          <div class="form-group">
            ...
            <input class="form-control {{ $errors->has('nip')?'is-invalid':'' }}"
                   type="text" name="nip" value="{{ old('nip') }}" placeholder="Nomor Induk Pegawai"/>
            @error('nip')<div class="form-error">{{ $message }}</div>@enderror
          </div>
          <div class="form-group">
            <label>No. Telepon</label>
            <input class="form-control" type="text" name="no_telp" value="{{ old('no_telp') }}" placeholder="Contoh: 08123456789"/>
          </div>
          <div class="form-group">
            <label>Bagian / Unit Kerja</label>
            <input class="form-control" type="text" name="bagian" value="{{ old('bagian') }}" placeholder="Contoh: Seksi Distribusi"/>
          </div>
          <div class="form-group">
            <label>Jabatan</label>
            <input class="form-control" type="text" name="jabatan" value="{{ old('jabatan') }}" placeholder="Contoh: Staf Statistisi"/>
          </div>
        </div>
      ...
      <div class="form-actions" style="display:flex;gap:12px;">
        <button type="button" class="btn-action btn-primary btn-lg"
          onclick="showConfirm({
            ...
            icon:'bi-person-check-fill',iconColor:'#059669',
            confirmText:'Ya, Buat Akun',confirmClass:'confirm-btn-success',
            onConfirm:function(){document.getElementById('form-create-karyawan').submit();}
          })">
          <i class="bi bi-person-plus-fill"></i> Buat Akun
        </button>
        <a href="{{ route('admin.karyawan.index') }}"
           style="display:inline-flex;align-items:center;gap:6px;padding:12px 20px;border-radius:10px;border:1.5px solid #E2E8F0;background:#F8FAFC;color:#64748B;font-size:14px;font-weight:600;text-decoration:none;">
