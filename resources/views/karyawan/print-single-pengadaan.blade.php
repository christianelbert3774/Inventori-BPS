...
<head>
  ...
  <title>Print Pengadaan #{{ str_pad($pengadaan->id, 5, '0', STR_PAD_LEFT) }}</title>
...
<div class="print-toolbar">
  <div class="print-toolbar-left">
    <span>📄 Print Permintaan Pengadaan #{{ str_pad($pengadaan->id, 5, '0', STR_PAD_LEFT) }}</span>
  </div>
  <div class="print-toolbar-right">
    <a href="{{ route('karyawan.pengadaan.index') }}" class="toolbar-btn btn-back">
      ← Kembali
    </a>
...
<div class="dokumen">
  ...
  <div class="kop">
    ...
    <div class="kop-text">
      ...
      <p>Bukti Permintaan Pengadaan Barang (SIBAS)</p>
    </div>
    <div class="kop-right">
      <div class="doc-label">No. Permintaan</div>
      <div class="doc-id">#{{ str_pad($pengadaan->id, 5, '0', STR_PAD_LEFT) }}</div>
  ...
  <div class="identitas">
    ...
    <div class="id-item">
      ...
      <span>{{ $pengadaan->created_at->locale('id')->isoFormat('D MMMM Y, HH:mm') }}</span>
    </div>
  </div>
  ...
  <div class="seksi">
    ...
    <div class="detail-grid">
      ...
      <div class="detail-box">
        <label>Status PBJ</label>
        <div class="val">
          @if($pengadaan->status_level2 === 'rejected' || $pengadaan->status_level2 === 'pending')
            <span style="color:var(--gray);">—</span>
          @elseif($pengadaan->status_level3 === 'completed')
            ...
            @endif
          @elseif($pengadaan->status_level3 === 'rejected')
      ...
      <div class="detail-box" style="grid-column: 1 / -1;">
        <label>Catatan</label>
        <div class="val">{{ $pengadaan->catatan }}</div>
      </div>
      @endif
    </div>
    ...
    <table class="tbl">
      ...
      <tbody>
        @foreach($pengadaan->details as $idx => $d)
          <tr>
            ...
            <td>{{ ($d->tipe_item === 'baru' && !$d->barang_id) ? $d->satuan_baru : ($d->barang->satuan ?? '-') }}</td>
          </tr>
        @endforeach
      </tbody>
    </table>
  ...
  <div class="doc-footer">
    ...
    <span>Pengadaan #{{ str_pad($pengadaan->id, 5, '0', STR_PAD_LEFT) }} · {{ $user->name }}</span>
