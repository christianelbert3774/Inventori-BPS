...
<body>
  ...
  <div class="print-toolbar">
    ...
    <div class="print-toolbar-right">
      ...
      <form method="GET" action="{{ route('karyawan.profil.print') }}" style="display:flex;gap:8px;align-items:center">
        ...
        <input type="month" name="bulan" value="{{ $bulan }}" class="bulan-select" onchange="this.form.submit()" />
      </form>
  ...
  <div class="dokumen">
    ...
    @php
      ...
      $approvedPg = $pengadaans->where('status_level3', 'completed')->count();
    @endphp
    <div class="ringkasan">
      <div class="rsum-item">
                ...
                <td>
                  @foreach($p->details as $d)
                    <div class="fw6">{{ $d->barang->nama_barang ?? $d->nama_barang_baru ?? 'Barang Baru' }}
                      <span style="font-weight:400;color:var(--gray)"> × {{ $d->jumlah }}
                        {{ $d->barang->satuan ?? $d->satuan_baru ?? '' }}</span>
                    </div>
                  @endforeach
                </td>
